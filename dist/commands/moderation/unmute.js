module.exports = {
    ...
    async execute(message, args, client, prefix) {
        ...
        try {
            if (!args[0])
                return client.embeds.notice(message.channel, await client.strings(message.guild, "cmd.unmute.userrequired"));
            if (!target)
                return client.embeds.error(message.channel, await client.strings(message.guild, "cmd.unmute.usernotfound"));
            if (target.roles.highest.position >= message.guild.me.roles.highest.position)
                return client.embeds.error(message.channel, (await client.strings(message.guild, "cmd.unmute.perms")).replace("$user", target.user.tag));
            client.con.query("SELECT * FROM guild_settings WHERE id = ?;", [message.guild.id], async (error, result) => {
                ...
                if (result[0].muted_id == null)
                    return client.embeds.notice(message.channel, await client.strings(message.guild, "cmd.unmute.rolemissing"));
