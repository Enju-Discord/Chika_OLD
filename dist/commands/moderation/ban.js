module.exports = {
    ...
    async execute(message, args, client, prefix) {
        ...
        try {
            if (!args[0])
                return client.embeds.notice(message.channel, await client.strings(message.guild, 'cmd.ban.userrequired'));
            if (!target)
                return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.ban.usernotfound'));
            if (target.id === client.user.id)
                return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.ban.userisbot'));
            if (target === message.guild.owner)
                return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.ban.userisowner'));
            if (!target.bannable)
                return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.ban.noban'));
            if (target === message.member)
                return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.ban.selfban'));
            if (!reason)
