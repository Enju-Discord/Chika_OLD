...
module.exports = {
    name: 'cmd.trap.name',
    description: 'cmd.trap.description',
    usage: 'cmd.trap.usage',
    args: true,
    dm: false,
    group: 'Image',
    cooldown: 10,
    ...
    async execute(message: any, args: any, client: any, prefix: any) {
        ...
        const image: string = await Neko.nsfw.trap().then(async img => img.url);
        ...
        if (message.channel.nsfw) {
            ...
            return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.trap.nsfw'));
        }
    }
}