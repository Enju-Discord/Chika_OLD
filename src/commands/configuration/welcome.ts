module.exports = {
    ...
    async execute(message: any, args: any, client: any, prefix: any) {
        client.con.query('SELECT * FROM guild_settings WHERE id = ?;', [message.guild.id], async (error: any, result: any) => {
            ...
            if (!channel) return client.embeds.notice(message.channel, await client.strings(message.guild, 'cmd.welcome.channelnotfound'));

            args.shift();
            if (args[1]) {
                welcome = args.join(' ');
                if (welcome.length > 800) return client.embeds.error(message.channel, await client.strings(message.guild, 'cmd.welcome.length'));
